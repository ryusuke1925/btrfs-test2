			 ...
			 u64 *prev_em_start)
{
	...
	while (cur <= end) {
		...
		if(extent_map_end(em) <= cur){
			printk("this is temporary code!! bad extent! em:[%llu %llu] current:[%llu] refcnt:[%d] pointer:[%p %p]",
				em->start, em->len, cur, refcount_read(&(em->refs)), em, *em_cached);
			frag=1;
		}
		////////////////////////////////
		...
		////////////////////////////////
		if(flag){
			printk("Confirmation em:[%llu %llu] current:[%llu] refcnt:[%d] pointer:[%p %p]",
				em->start, em->len, cur, refcount_read(&(em->refs)), em, *em_cached);
		}
		////////////////////////////////
